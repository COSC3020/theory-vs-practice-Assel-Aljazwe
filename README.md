...
**Tree Imbalance**: In a binary search tree that is not well-balanced, the search path typically is much longer than log(n). In an unbalanced tree, the structure is often very odd and uneven with a worst case height of n. The worst case complexity you would expect would be O(n) search time. Meaning this search time is no longer logarithmic but linear, which leads to much longer search paths and therefore, increased search times which explains the jump to 100 seconds.<br />
